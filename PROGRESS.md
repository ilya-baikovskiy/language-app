...
1. Реализовать SM-2 (`scheduleNext`) в коде по таблице выше — это чистая
   функция, не UI, можно начинать без остальных пунктов.
2. Решить и реализовать AI-шаг генерации клоуз-фразы (см. «Источник фразы»
   ...
   верифицировались правки Bottom Sheet в этой же сессии.
3. Экран очереди (`LearnPage.tsx` сейчас — просто список, кнопка «Начать»
   `disabled`) + экран практики по утверждённому артефакту.
4. Голосовой ввод — распознавание речи, отдельная техническая задача,
   ...
   результат.
5. Ничего из этого не запушено и не задеплоено — это первая реализация с
   нуля поверх уже существующих `savedWord.ts`/`useSavedWords.ts`/
   `LearnPage.tsx`.
