...
- Замедлить дефолтную скорость сгенерированной озвучки (сейчас нормальный темп,
  пользователь попросил чуть медленнее) — через параметр `speed` в вызове
  `gpt-4o-mini-tts`, с перегенерацией audio+timestamps.

## Дальше (кандидаты, порядок не зафиксирован)

1. **AI-генерация Bottom Sheet контента** для оставшихся ~100 неразмеченных вручную слов
   урока, по шаблону из `AI_PIPELINE.md` (полное предложение как контекст). По аналогии с
   аудио-пайплайном — одноразовый скрипт + merge в `sampleLesson.ts`/новый JSON, а не
   live-вызовы на каждый клик (решение уже склонялось в эту сторону, см. ниже).
2. **Входной UI-пайплайн** (материал/ссылка/тема → настройки уровня и размера → текст +
   озвучка) — существенно больше по объёму: нужен реальный backend для рантайм-генерации
   новых уроков (в отличие от пункта 1, где урок фиксирован и генерация одноразовая).
   Сначала статический UI шагов (по аналогии с Этапом 1 самого ридера), backend — когда
   решим окончательно провайдера текста.
