...
**Порядок ближайшего продолжения уточнён пользователем:** перед реализацией
следующего пункта сначала открыть и визуально проверить последний свёрстанный
каркас тренировки. После этой сверки следующий кодовый этап остаётся прежним —
SM-2 (`scheduleNext`) и запись результата повторения.
