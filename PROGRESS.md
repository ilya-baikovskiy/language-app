...
## Текущий статус (2026-07-28): тир 2 синхронизирован с тир 1 + план тренировки слов (артефакты, БЕЗ кода)

Сессия обрывается по лимиту — пользователь продолжит с другим AI-инструментом.
Этот раздел фиксирует всё разобранное, чтобы продолжающий не начинал с нуля.
Ниже — **план тренировки**, полностью проговорённый и утверждённый пользователем
по частям, но **НЕ реализованный в коде** — ни одной строчки, только 3 HTML-артефакта
(ссылки ниже) и решения словами. Следующий шаг для исполнителя — код, не обсуждение.
...
1. Реализовать SM-2 (`scheduleNext`) по таблице выше — чистая функция,
   ...
   происходить).
2. Решить и реализовать AI-шаг генерации клоуз-фразы (см. «Источник фразы»
...
3. Реальный тап-по-слову — подключить к существующему механизму аннотаций
   вместо заглушки `setTappedWord`.
4. Голосовой ввод — распознавание речи, отдельная техническая задача,
   формат ответа (текст vs голос) должен давать один и тот же 3-вердиктный
   результат.
5. Живой прогон в браузере на `vercel dev`/проде — каркас не проверялся
   визуально, только тестами и сборкой.
