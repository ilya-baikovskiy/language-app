...
- Pipeline A (AI-генерация карточек из тем×стран, `07_AI_CONTENT_GENERATION_PIPELINE.md`
  §2) — пользователь дал добро начинать. Отдельно от Phase 7/adaptive ranking (умного
  алгоритма интересов) — тот остаётся за флагом, не трогаем сейчас. Открытый вопрос,
  который ещё не решён: сколько карточек генерировать за один прогон / когда триггерить
  перегенерацию пула (нужно решение пользователя, см. следующую сессию).
