...
//
// Каркас экрана практики (2026-07-28, см. PROGRESS.md) — без SM-2/AI-шага,
// но сама вёрстка/переходы должны реально работать на настоящих SavedWord.
...
describe('TrainingPracticeView', () => {
  ...
  it('тап по соседнему слову показывает заглушку, не трогая пропуск', () => {
    render(<TrainingPracticeView words={[makeWord()]} onExit={vi.fn()} />);
    const neighbor = screen.getAllByText('βοηθά')[0];
    fireEvent.click(neighbor);
    expect(screen.getByText(/появятся здесь/)).toBeTruthy();
  });
  ...
  it('раскрытие подсказки необратимо помечает попытку, даже после того как её снова спрятали', () => {
    ...
    expect(screen.getByText(/Подсказка была открыта/)).toBeTruthy();
  });
  ...
  it('без сохранённого предложения — честный запасной вариант без клоуза, без крэша', () => {
    render(<TrainingPracticeView words={[makeWord({ contextSource: undefined, contextTranslation: undefined })]} onExit={vi.fn()} />);
    expect(screen.getByText('κατασκευή')).toBeTruthy();
