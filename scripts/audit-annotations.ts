...
function auditSections(word: string, sections: DetailSection[]): Finding[] {
  ...
  sections.forEach((section, i) => {
    ...
    if (section.type === 'table') {
      ...
      // Реальный баг: таблица времён смешивала лица вместо форм одного лица —
      // «я/я/они/они» вместо «я, настоящее/прошедшее/будущее». Один и тот же
      // ярлык дважды в таблице — верный признак, что смешали две оси сравнения
      // (лицо × время), а должна остаться одна.
      const labels = section.rows.map((r) => r[0]?.trim().toLowerCase());
      if (new Set(labels).size !== labels.length) {
        out.push({
          severity: 'fail',
          rule: 'таблица сравнивает одну ось (не смешивает лица и времена)',
          where: at,
          detail: `повторяющиеся ярлыки строк: ${labels.join(', ')}`,
        });
      }
