...
> **Разрешённые расхождения (2026-07-22).** В `screenshots/` лежат два набора:
> исходный (`00`–`05.jpg`, на них ссылается §10) и более поздний, подробный
> (`Screenshot 2026-07-22 …png`). В двух местах они противоречат друг другу и
> тексту ниже. Решения приняты и реализованы:
>
> 1. **Строка «Словарная форма» наверху не показывается** (как в §5 и в `01-pige-short.jpg`),
>    хотя поздние PNG её показывают. Начальная форма мешает читать; если она полезна,
>    её место — в подробностях. Слот под переводом при этом остаётся, но с другими
>    ярлыками: `В этом предложении` и `Другое значение`.
> 2. **Таблица времён глагола — в лице «я»** (как в §5, §11 и `02-pige-details-top.jpg`),
>    хотя поздние PNG используют «он / она».
>
> Остальное поздние PNG уточняют, не противореча: таблицы без строки заголовков
> (роль несёт первая ячейка строки), хвостовой абзац под таблицей, `grammarNote`
> обычным абзацем без плашки, «Подробнее» в две строки, футер из двух элементов.
