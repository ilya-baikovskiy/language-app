...
## 11. Другие расхождения исходного контекста с реальным кодом

Документ `_existing_project_context/01_product_learning_ux.md` в целом точно
описывает продукт и переход на Bottom Sheet v2 per-token. Тем не менее при сверке
с реальным кодом нашлись более мелкие неточности/уточнения:
...
2. **`coverImage?: string` в `Lesson`** упомянут в типе, но не используется нигде в
   ...
   в типе.
3. Ни в `PROGRESS.md`, ни в `AI_PIPELINE.md`, ни в `PLAN.md`/`DESIGN.md` нет упоминаний
...
4. Явного "экрана Bottom Sheet handoff" каталога в момент discovery в дереве —
