...
interface GeneratedCardCandidate {
  ...
  emoji: string; // 1-2 эмодзи (тема + страна/регион), см. правило ниже
  topicIds: string[];
...
### Правило заголовка: называть место напрямую

`editorialTitleRu` должен называть конкретное место/страну явно, а не обобщённо —
«Санторини», «Швейцария», «Германия», не «остров», «страна», «одна из стран». Обобщённые
заголовки менее цепляющие и хуже различимы в ленте, когда несколько карточек про разные
страны в одной теме. Плохо: *«Страна, которая голосует каждые три месяца»*. Хорошо:
*«Швейцария, которая голосует каждые три месяца»*. Плохо: *«Как строили дома на
вулканическом острове»*. Хорошо: *«Как строили дома на вулканическом острове Санторини»*.
Это применимо и к seed-карточкам, написанным вручную (см. `content-ideas.v1.json`), и к
`editorialTitleRu`, который будет генерировать AI по этому pipeline.

`emoji` подбирается AI как часть кандидата (по тем же принципам, что и заголовок/описание) —
обычно комбинация «тема + страна» (🌋🇬🇷, 🗣️🇨🇭), не требует отдельного шага/картинки.
